...
|---|---|
| [5.4.1 Entry Point & Stack Composition](5.4.1-entry-point/) | The main stack that links all the modules together in the right order (Includes: CDK App, AwsplaceStack, RaftDbStagingStack) |
| [5.4.2 VPC & Networking](5.4.2-vpc-networking/) | Network setup using public subnets and no NAT Gateway to save costs (Includes: VPC, subnets, security groups) |
| [5.4.3 DynamoDB Database Layer](5.4.3-database-dynamodb/) | A simple single-table design using composite keys (Includes: 4 on-demand DynamoDB TableV2s) |
| [5.4.4 ECS Fargate Compute Layer](5.4.4-ecs-compute/) | Where the Go app and RaftDB run behind an HTTPS load balancer (Includes: Fargate cluster, task definitions, ALB) |
| [5.4.5 Storage: ECR & S3](5.4.5-storage-ecr/) | Where we store Docker images and app files (Includes: ECR repository, 2 imported S3 buckets) |
| [5.4.6 IAM Roles & Security](5.4.6-iam-security/) | Setting up least-privilege permissions for the containers and Lambda (Includes: 3 IAM roles) |
| [5.4.7 Lambda, API Gateway & Amplify](5.4.7-lambda-apigw-amplify/) | Serverless auth functions and frontend hosting (Includes: Lambda, HTTP API v2, Amplify app) |
| [5.4.8 RaftDB Infrastructure & Route 53](5.4.8-raftdb-route53/) | DNS, certificates, and the RaftDB staging environment (Includes: Route 53, ACM cert, staging stack) |
